...
Public Function ProgressBar(ArqDir As String, lDone As Long)
    ...
    End If
    lDoneBytes = lDone
    If Tamanho = 0 Then: Tamanho = 5000
    Inicio.PB.Value = (lDone / Tamanho) * 100
    
    Dim NomeArq() As String
    NomeArq = Split(ArqDir, "\")
    Inicio.Status.Caption = "Baixando " & NomeArq(UBound(NomeArq))
End Function

Public Function CorrigePeso(Bytes) As String
    On Error GoTo hell
    If Bytes >= 1073741824 Then
        CorrigePeso = Format(Bytes / 1024 / 1024 / 1024, "#0.0") _
             & " GB"
    ElseIf Bytes >= 1048576 Then
        CorrigePeso = Format(Bytes / 1024 / 1024, "#0.0") & " MB"
    ElseIf Bytes >= 1024 Then
        CorrigePeso = Format(Bytes / 1024, "#0.0") & " KB"
    ElseIf Bytes < 1024 Then
        CorrigePeso = Fix(Bytes) & " B"
    End If
...
Public Function DownloadAFile(sFileName As String, sURLfrom As String, bProgressBar As Boolean) As Boolean
                ...
                If hInternetOpen Then
                    ...
                    Do While bDoLoop
                        ...
                        If bProgressBar Then
                            Call ProgressBar(sFileName, lTotalBytesRead)
                        End If
